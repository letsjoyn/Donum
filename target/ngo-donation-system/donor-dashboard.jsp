...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
        ...
        <%@ include file="/WEB-INF/header.jspf" %>
            ...
            <% User user=(User) session.getAttribute("user"); if (user==null || !"Donor".equals(user.getRole())) {
                response.sendRedirect("login"); return; } DonationDAO dDao=new DonationDAO(); List<Donation> myDonations
                = dDao.getDonationsByDonor(user.getUserId());
                ...
                List<Campaign> campaigns = cDao.getActiveCampaigns();
                    ...
                    <c:if test="${param.success == '1'}">
                        <div class="alert alert-success"><i class="fas fa-check-circle"></i> Thank you! Your donation
                            was recorded successfully.</div>
                    </c:if>
                                ...
                                <div id="kindSection" style="display:none;">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="itemName" name="itemName"
                                            placeholder="e.g. Rice, Blankets, Medicines" maxlength="100">
                                    </div>
                                </div>
                                        ...
                                        <table>
                                            ...
                                            <tbody>
                                                <c:forEach var="d" items="${myDonations}">
                                                    <tr>
                                                        <td>
                                                            <fmt:formatDate value="${d.donationDate}"
                                                                pattern="dd MMM yyyy" />
                                                        </td>
                                                        <td><span
                                                                class="badge badge-${fn:toLowerCase(d.type)}">${d.type}</span>
                                                        </td>
                                                        <td>
                                                            <c:choose>
                                                                <c:when test="${d.type == 'Cash'}">
                                                                    <strong>&#8377;${d.amountOrQuantity}</strong>
                                                                </c:when>
                                                                <c:otherwise>${d.amountOrQuantity} x
                                                                    ${fn:escapeXml(d.itemName)}</c:otherwise>
                                                            </c:choose>
                                                        </td>
                                                        <td>${fn:escapeXml(d.campaignName)}</td>
                                                        <td><span
                                                                class="badge badge-${fn:toLowerCase(d.status)}">${d.status}</span>
                                                        </td>
                                                        <td>
                    ...
                    <c:if test="${not empty campaigns}">
                        <div style="margin-top: 1.5rem;">
                            <h3 class="section-title" style="font-size:1.2rem;"><i class="fas fa-bullhorn"></i> Active
                                Campaigns</h3>
