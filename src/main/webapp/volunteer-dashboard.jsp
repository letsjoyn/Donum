...
<%
    ...
    if (user == null || !"Volunteer".equals(user.getRole())) { response.sendRedirect("login"); return; }
...
<div class="main-content">
    <div class="page-header">
        ...
        <p>Log field distributions and track your impact, ${fn:escapeXml(sessionScope.user.fullName)}.</p>
    </div>
    ...
    <c:if test="${param.success == '1'}">
        <div class="alert alert-success"><i class="fas fa-check-circle"></i> Distribution logged successfully. Inventory updated via trigger.</div>
    </c:if>
    ...
    <div class="content-grid">
        ...
        <div class="glass-card" style="height:fit-content;">
            ...
            <form action="distribute" method="post">
                <div class="form-group">
                    <label for="requirementId"><i class="fas fa-map-marker-alt"></i> Requirement (Destination)</label>
                    <select id="requirementId" name="requirementId" required>
                        ...
                        <c:forEach var="r" items="${reqs}">
                            <option value="${r.requirementId}">
                                [${r.urgency}] ${fn:escapeXml(r.location)} - ${fn:escapeXml(r.itemName)} (need ${r.quantityNeeded})
                            </option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label for="quantity"><i class="fas fa-sort-numeric-up"></i> Quantity to Distribute</label>
                    <input type="number" id="quantity" name="quantity" required min="1" max="100000" step="1" placeholder="Amount to deliver">
                </div>

                <div class="form-group">
                    <label for="notes"><i class="fas fa-sticky-note"></i> Field Notes (optional)</label>
                    <textarea id="notes" name="notes" placeholder="Conditions on ground, difficulties, etc." maxlength="500" rows="2"></textarea>
                </div>

                <div class="alert alert-info" style="margin-bottom:1rem;">
                    <i class="fas fa-info-circle"></i>
                    <span>Submitting triggers automatic inventory deduction via database trigger (<code>trg_after_distribution_insert</code>).</span>
                </div>
        ...
        <div class="glass-card">
            <h3 class="section-title"><i class="fas fa-exclamation-triangle"></i> Pending Requirements</h3>
            <c:choose>
                <c:when test="${empty reqs}">
                    <div class="empty-state"><i class="fas fa-check-circle"></i><p>All requirements fulfilled!</p></div>
                </c:when>
                <c:otherwise>
                    <div class="scroll-area">
                        <table>
                            <thead><tr><th>Location</th><th>Item</th><th>Needed</th><th>Fulfilled</th><th>Urgency</th></tr></thead>
                            <tbody>
                                <c:forEach var="r" items="${reqs}">
                                    <tr>
    ...
    <div class="glass-card" style="margin-top:1.5rem;">
        ...
        <c:choose>
            <c:when test="${empty myDist}">
                <div class="empty-state"><i class="fas fa-truck"></i><p>No distributions logged yet.</p></div>
            </c:when>
            <c:otherwise>
                <div class="scroll-area">
                    <table>
                        <thead><tr><th>Date</th><th>Location</th><th>Item</th><th>Qty</th><th>Notes</th></tr></thead>
                        <tbody>
                            <c:forEach var="d" items="${myDist}">
                                <tr>
                                    ...
                                    <td>${d.quantity}</td>
                                    <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${fn:escapeXml(d.notes)}</td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
